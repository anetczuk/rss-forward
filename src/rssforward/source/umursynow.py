...
MAIN_NAME = "UM Ursynów"
MAIN_URL = "https://ursynow.um.warszawa.pl"


class UMUrsynowGenerator(RSSGenerator):
...
def get_content(items_num=20, html_output=None):
    ...
    for full_url in news_links:
        add_news(feed_gen, full_url, html_output)
...
def extract_news_data(news_url=None, content=None):
    ...
    article = soup.find_all("div", attrs={"id": "column-2"})[0]

    article_title_list = article.find_all("h1", attrs={"class": "article-title"})
    if not article_title_list:
        _LOGGER.warning("unable to get event from: %s", news_url)
        return None
    title_text = article_title_list[0].text

    article_element_list = article.find_all("button")
    for item in article_element_list:
        item.clear()

    article_element = article.find_all("span", attrs={"class": "header-back-to"})[0]
    article_element.clear()

    article_element = article.find_all("div", attrs={"class": "article-tools"})[0]
    article_element.clear()
