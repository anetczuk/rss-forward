...
def get_offers_content(label, filter_url, filter_items, throw=True):
    ...
    for offer in json_offers_list:
        add_offer(feed_gen, offer)

    content = dumps_feed_gen(feed_gen)
    return content


def add_offer(feed_gen, data_dict):
    ...
    feed_item.title(f"Job: {offer_company} - {offer_title}")
    ...
    # data_string = convert_to_html(data_string, preserve_newline=True)
    item_desc = f"""\
{item_desc}
